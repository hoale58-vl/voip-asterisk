...
const int listOutputGpio[8] = {3, 2, 1, 0, 16, 15, 7, 9};
std::map<std::string, int> hostLookupTable;
static char remoteHost[8][4] = {"Pi1", "Pi2", "Pi3", "Pi4", "Pi5", "Pi6", "Pi7", "Pi8"};

static void setScheduler();
static int configureServer();
...
int main(int argc, char *argv[])
{
	...
	wiringPiSetup ();
	for (int i=0; i<8; i++){
  		pinMode (listOutputGpio[i], OUTPUT) ;
		digitalWrite(listOutputGpio[i], LOW) ;

		struct hostent *he;
		if ( (he = gethostbyname(remoteHost[i]) ) == NULL ) {
			printf("Cannot get host by name %s\n", remoteHost[i]);
			exit(1); /* error */
		}
		struct in_addr *tmp = (struct in_addr *)*he->h_addr_list;
		char* ipAddr = inet_ntoa(*tmp);
		hostLookupTable[ipAddr] = i;
	}
...
static int configureServer() {
	...
	if(bind(server, (struct sockaddr*)&sin, sizeof(sin)) == -1) {
		logMessage("Error: port already used, choose another\n");
