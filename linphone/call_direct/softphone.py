...
class SoftPhone:
    ...
    def call_state_changed(self, core, call, state, message):
        if state == linphone.CallState.IncomingReceived:
            logging.info('IncomingReceived')
            params = core.create_call_params(call)
            params.audio_direction = stream_params["RecvOnly"]
            core.accept_call_with_params(call, params)
            core.add_to_conference(call)
        elif state == linphone.CallState.Released:
            pass
        elif state == linphone.CallState.Paused:
            # core.resume_call(call)
            pass
        else:
            logging.info(call_state_dict[state])

    def make_call(self, username):
    ...
    def update_call(self, username, enable):
        for current_call in self.core.calls:
            if current_call.remote_address.username == username:
                params = self.core.create_call_params(current_call)
                if params.audio_direction == stream_params["RecvOnly"]:
                    if enable:
                        logging.info('SendRecv {}'.format(username))
                        params.audio_direction = stream_params["SendRecv"]
                    else:
                        return
                elif params.audio_direction == stream_params["SendOnly"]:
                    if enable:
                        return
                    else:
                        logging.info('Terminating {}'.format(username))
                        self.core.terminate_call(current_call)
                elif params.audio_direction == stream_params["SendRecv"]:
                    if enable:
                        return
                    else:
                        logging.info('RecvOnly {}'.format(username))
                        params.audio_direction == stream_params["RecvOnly"]
                self.core.update_call(current_call, params)
                return
        if enable:
            self.make_call(username)
